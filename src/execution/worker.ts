...
async function main(): Promise<void> {
  ...
  const mediaInstructions = request.item.expectedOutputRoles?.length
    ? `\n\nProduce at least ${request.item.minimumAssetSets ?? 1} complete candidate AssetSets with different content. Put candidate bytes under .factory-media/ and write .factory-assets.json at the checkout root. The JSON must be {"sets":[{"id":"candidate-a","members":[{"role":"image","path":".factory-media/candidate-a/image.png","mediaType":"image/png","destination":"approved/image.png"}],"provenance":{"source":"source path or generated","rights":"basis for repository use","visibility":"repository","lineage":["source path"]}}]}. Include every expected role in each set. .factory-media/ and .factory-assets.json are the only staging exceptions to owned paths. Do not write final destinations directly. Candidate files and the manifest are staging outputs; do not commit them. The controller will preserve the exact bytes for human review and selection.\nSource assets: ${(request.item.sourceAssets ?? []).join(", ")}\nExpected output roles: ${request.item.expectedOutputRoles.join(", ")}`
    : "";
