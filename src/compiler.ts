...
export class CodexPlanningModel implements PlanningModel {
  ...
  async reviewResult(request: {
    ...
    const identityInstructions =
      'The result identity is a Git tree. Delivery observations separately name every Git commit and Git tree; never compare them as the same object type. Command pass evidence is an ordered array of canonical receipts. Each receipt names its stable zero-based index, command, successful exit code 0, and exact result tree, produced only after Factory verified the result commit resolves to that tree. Asset capture receipts inside Delivery observations are controller-generated only after Factory verifies each complete declared AssetSet member beneath .factory-media/ and imports its exact bytes. A capture receipt proves .factory-assets.json origin only when its declarationPath and declarationDigest fields are present; those fields mean Factory independently parsed that regular manifest and matched it to the harness AssetSets. Asset selection receipts are controller-generated from validated atomic state and bind the selected set digest, recorded actor (the OS username when the caller omitted one), controller-derived invocation surface, time, destinations, downstream bindings, and an optional reason only when present. An absent receipt, absent declaration fields, unrecorded selection surface, or absent reason proves nothing about that missing fact. Sources whose path begins with Work Item Git delta are supervisor-generated exact result evidence: they bind accepted path ownership and that item\'s execution base, actual result base, result commit/tree, integrated commit/tree, changed paths, and raw patch excerpts. "Controller hydration receipt" is supervisor-generated evidence that Factory completed fresh-clone hydration and exact selected-byte verification before this review. Treat each such path as an allowed supplied source path. Use those sources only for criteria their exact content proves. Copy quotes exactly as serialized; never decode an escaped string into a quote. ';
    const prompt =
