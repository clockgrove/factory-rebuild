...
### Planned compiler graph review

[Trunk issue #19](https://github.com/clockgrove/factory-rebuild/issues/19) defines the intended routine path: one structured compiler output from a complete, pinned source packet, deterministic graph checks, then one independent LLM review of that same packet and proposed graph. The reviewer must cite source-backed findings for missing Objective obligations, unsupported scope, citation defects, dependency or ownership mistakes, and unobservable acceptance. Deterministic checks remain authoritative for machine-checkable facts; the reviewer cannot grant authority or silently edit the graph.

A concrete finding permits at most one evidenced compiler revision, followed by deterministic revalidation and independent re-review. A clean validated and reviewed graph proceeds without routine human approval. Human inspection of early read-only previews, including the Clockgrove draft, is a temporary calibration scaffold. After the bounded revision, unresolved source conflict, missing authority, material defects or disagreement, or an explicitly human-owned decision triggers one specific question with a recorded fallback reason. Do not add a recursive judge/repair pipeline or a generic request to approve every graph. The read-only preview and independent graph review are implemented in #19; command authority and result-level acceptance are completed separately in #20.
...
The `v0.1.0` one-immutable-package combined disposable gate passed; [BUILD-STATUS.md](BUILD-STATUS.md) records its package digest and final integrated head. The operator-approved pre-pilot trunk closure was issues [#19–#25 and #28](https://github.com/clockgrove/factory-rebuild/issues). A later audit of Clockgrove W0-001 found that its accepted base has no pnpm package or lockfile, while the published validator requires them before planning; focused [#44](https://github.com/clockgrove/factory-rebuild/issues/44) closes this greenfield command-authority gap. The resulting exact-artifact gates exposed three bounded review gaps: [#46](https://github.com/clockgrove/factory-rebuild/issues/46) grounds independent result findings in supplied evidence, [#48](https://github.com/clockgrove/factory-rebuild/issues/48) preserves malformed graph review as an explicit human decision rather than a false clean pass, and [#51](https://github.com/clockgrove/factory-rebuild/issues/51) supplies authoritative attempt provenance to Work Item result review, including the immutable worker base and integration head observed at attempt start across later native replay. Their combined fix requires a new immutable public artifact and a complete fresh installed disposable gate before [#26](https://github.com/clockgrove/factory-rebuild/issues/26), the exact-public-artifact Clockgrove pilot; an older paused run or repeated result override is not acceptance. Later capability branches, repository cutover, and trunk-complete claims remain on hold until those gates pass. A clean refactor that wires known variation-point contracts to real behavior is welcome when it reduces a concrete gap. The first-release agent interface is the CLI plus installed skills over one application layer, with easy harness-agnostic installation and no MCP prerequisite. A later MCP adapter needs a demonstrated agent-integration need and must not duplicate lifecycle logic. This public repository, including contributor instructions, fixtures, and acceptance evidence, must remain sufficient for unrelated contributors; `AGENTS.md` guides building Factory, while packaged skills guide using it.
