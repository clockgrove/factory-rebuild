...
`src/runner.ts` composes the compiler (`src/compiler.ts`), scheduler (`src/scheduler.ts`), local execution (`src/execution/`), validator (`src/validation.ts`), GitHub gateway (`src/github.ts`), and delivery strategies (`src/delivery/`). `src/contracts.ts` holds the narrow outer contracts. The installed CLI writes private schemaVersion 1 configuration and one atomic snapshot outside the target checkout. It reads no archived Factory config or state.
