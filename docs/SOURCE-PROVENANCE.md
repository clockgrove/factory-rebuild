...
## Private Codex provider temp root (#97)

The preserved fresh installed v0.1.17 gate and the current Codex 0.156.0 runtime requirement supplied this correction. The outer qualification harness intentionally presented its own app-server socket directory as an inaccessible read-only mount; the nested worker therefore could not satisfy Codex's user-owned `0700` socket-directory requirement. No archived Factory implementation, provider wrapper, sandbox policy, or retry behavior was inspected or copied. The clean local harness now derives a traversal-safe per-attempt path beneath Factory's private harness state, validates ownership, type, real path, and exact mode, and supplies it only as the detached worker's `TMPDIR`. Credential isolation, model policy, process ownership, and the atomic lifecycle remain unchanged.
