...
sha256sum clockgrove-factory-0.1.20.tgz > SHA256SUMS
```

Inspect the tarball file list for the manifest, installed skills, CLI, license, logo, notices, and bundled production dependency tree. In a separate empty prefix, install the tarball with `npm install --offline --ignore-scripts --prefix /absolute/private/check-prefix ./clockgrove-factory-0.1.20.tgz` using an empty npm cache; verify `factory help`, compare every installed bundled package version with `package-lock.json`, and check that notices cover the same tree. Record `git rev-parse HEAD`, package version, tarball SHA-256, and the passing CI run. Create a protected `v0.1.20` tag at that same commit and attach both `clockgrove-factory-0.1.20.tgz` and `SHA256SUMS` to a public GitHub Release. Record the expected SHA-256 outside the mutable Release assets, in [BUILD-STATUS.md](BUILD-STATUS.md). This procedure does not itself publish or tag anything.
...
```sh
codex plugin marketplace add clockgrove/factory-rebuild --ref v0.1.20
codex plugin add factory@clockgrove
gh release download v0.1.20 --repo clockgrove/factory-rebuild \
  --pattern clockgrove-factory-0.1.20.tgz --pattern SHA256SUMS
sha256sum --check SHA256SUMS
# Also compare the digest with the independently recorded release value in BUILD-STATUS.md.
npm install --offline --ignore-scripts --prefix /absolute/private/factory-prefix ./clockgrove-factory-0.1.20.tgz
