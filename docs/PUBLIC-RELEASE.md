...
sha256sum clockgrove-factory-0.1.2.tgz > SHA256SUMS
```

Inspect the tarball file list for the manifest, installed skills, CLI, license, logo, notices, and bundled production dependency tree. In a separate empty prefix, install the tarball with `npm install --offline --ignore-scripts --prefix /absolute/private/check-prefix ./clockgrove-factory-0.1.2.tgz` using an empty npm cache; verify `factory help`, compare every installed bundled package version with `package-lock.json`, and check that notices cover the same tree. Record `git rev-parse HEAD`, package version, tarball SHA-256, and the passing CI run. Create a protected `v0.1.2` tag at that same commit and attach both `clockgrove-factory-0.1.2.tgz` and `SHA256SUMS` to a public GitHub Release. Record the expected SHA-256 outside the mutable Release assets, in [BUILD-STATUS.md](BUILD-STATUS.md). This procedure does not itself publish or tag anything.
...
npm install --offline --ignore-scripts --prefix /absolute/private/factory-prefix ./clockgrove-factory-0.1.2.tgz
...
Issue #26 is a separate private adopter smoke using the new exact published artifact after its public disposable gate succeeds. The `v0.1.0` proof is historical and does not qualify changed `v0.1.2` code. The v0.1.2 artifact is published, but its disposable gate is blocked at read-only graph review as recorded in [BUILD-STATUS.md](BUILD-STATUS.md).
