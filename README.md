...
**Status:** active public rebuild. The local Codex SDK path, dependency DAG, ordinary lifecycle, and regular pull-request delivery have passed installed-package disposable gates. Native stacks, media, LFS delivery, and the first live Clockgrove Objective are being completed in [the public project](https://github.com/orgs/clockgrove/projects/2). The current package is a development candidate, not a published release.
...
The [Factory Rebuild project](https://github.com/orgs/clockgrove/projects/2) tracks one acceptance issue per trunk slice and later capability branches. [Source provenance](docs/SOURCE-PROVENANCE.md) records which behaviors were reimplemented after inspecting the archived Factory code. The archived source is reference material; this repository is a clean implementation.
